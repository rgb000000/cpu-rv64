...
// ref from https://github.com/OpenXiangShan/XiangShan/blob/master/src/main/scala/xiangshan/backend/fu/SRT16Divider.scala
...
class DividerIO(implicit val p: Parameters) extends Bundle{
...
class Divider(implicit val p: Parameters) extends Module {
  ...
  divider.io.isW := ctrlReg.isW
  divider.io.isHi := ctrlReg.isHi
  divider.io.src(0) := io.in.bits.src(0)
  divider.io.src(1) := io.in.bits.src(1)
  io.in.ready := divider.io.in_ready

  io.out.valid := divider.io.out_valid
  io.out.bits := divider.io.out_data
