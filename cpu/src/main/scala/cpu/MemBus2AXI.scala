...
class MemBus2AXI(implicit p: Parameters) extends Module{
  ...
  val resp_data_reg = Vec(p(NBank), Reg(UInt(p(XLen).W)))
  ...
  when(state === idle){
    ...
    ar.bits.len := (p(CacheLineSize) / AXI4Parameters.dataBits).toInt.U
    ...
    aw.bits.len := (p(CacheLineSize) / AXI4Parameters.dataBits).toInt.U
    ...
    w.bits.strb := Vec((p(XLen) / 8).toInt, 1.U(1.W)).asUInt() // 0xff
