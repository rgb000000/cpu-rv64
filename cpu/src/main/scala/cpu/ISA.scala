...
//  def putch  = BitPat("b000000000000_00000_000_00000_1111011") // 7b  putch
  ...
  def rocc_mvin     = BitPat("b1000000_?????_?????_110_?????_0001011") // dcache数据move到spad中 spad[rs1] = mem[rs2的值] len固定是16
  def rocc_mvout    = BitPat("b1000001_?????_?????_110_?????_0001011") // spad数据move到dcache中 mem[rs2的值] = spad[rs1] len固定是16
